...
public class ProductRepository {
    ...
    public void findCustomers(int i) {
        EntityManager em = emFactory.createEntityManager();
        Product productNew = em.createQuery("select p from Product p join fetch p.customer where p.id = :id", Product.class)
                .setParameter("id", 1)
                .getSingleResult();
        System.out.println(productNew);
        System.out.println(productNew.getCustomer().getName());
